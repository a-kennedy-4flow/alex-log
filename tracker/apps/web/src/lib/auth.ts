...
interface Session {
  /**
   * The ID token rather than the access token. Because a) the authorizer checks
   * the audience which only an ID token carries. b) the access token holds
   * neither the email nor the name that the handler reads. c) the two are
   * issued together so nothing is lost by carrying the wider one.
   */
  idToken: string
  claims: Claims
  expiresAt: number
...
export async function apiToken(): Promise<string | null> {
  ...
  if (Date.now() < session.expiresAt - RENEW_MARGIN_MS) return session.idToken
