...
// sent. The file is 660 kB and the lists compress to about 60 kB so the upload
// stays small and the Lambda needs no spreadsheet parser.
//
...
import { NotATracker, readCatalogueFrom, type ParsedCatalogue } from '@timesheets/workbook-reader'
...
const current = ref<{ updatedAt: string; projects: number } | null>(null)

async function loadCurrent(): Promise<void> {
  if (!usingApi) return
  try {
    const value = (await api.catalogue()) as { updatedAt?: string; projects?: unknown[] }
    current.value = {
...
const summary = computed(() => {
  ...
  if (!value) return []
  return [
    { label: t('admin.projects'), value: value.projects.length },
...
async function send(): Promise<void> {
  ...
  failure.value = null
  try {
    const saved = await api.putCatalogue(value)
