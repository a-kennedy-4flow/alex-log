...
export function setCatalogue(data: CatalogueInput): void {
  ...
  // 350 workday ids appear more than once in the shipped list. The copies
  // differ only in cost centre and project number and never in title or
  // business line or type. The first wins because XLOOKUP in the tracker also
  // returns the first match. Keeping them would offer the user the same choice
  // twice and would repeat a key in the picker.
