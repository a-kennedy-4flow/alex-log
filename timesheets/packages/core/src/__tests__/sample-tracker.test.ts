...
describe('the region in the file name', () => {
  it('drops the entity number and upper cases the rest', () => {
    expect(regionOf('01_DE_Berlin')).toBe('DE_BERLIN')
    expect(regionOf('10_AT_Vienna')).toBe('AT_VIENNA')
    expect(regionOf('13_ES_Madrid')).toBe('ES_MADRID')
  })

  it('collapses a city holding a separator', () => {
    expect(regionOf('01_DE_Ruesselsheim / Bad Nauheim')).toBe('DE_RUESSELSHEIM_BAD_NAUHEIM')
  })

  it('does not yet match the shipped August file name', () => {
    // That file is named `..._projecttracker_DE.xlsm` from location
    // `01_DE_Berlin`. The stated rule says the city belongs in the name. The
    // two disagree and the question is open.
    expect(regionOf('01_DE_Berlin')).not.toBe('DE')
  })
})
